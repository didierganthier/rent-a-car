...
console.log(reservation)
  return (
    ...
    <p className="text-center text-gray-500">Go to the home page and book a car.</p>
    <img src="https://cdn-icons-png.flaticon.com/512/9841/9841554.png" alt="undraw-Empty-re-9gll" border="0" className="w-[300px] h-[300px] mt-5" />
    </div>
  ) : (
    <div className="flex flex-row flex-wrap gap-5 md:flex-nowrap md:gap-0 md:flex-col w-full mb-12">
      {reservation.map((info) => {
        return cars.map(car => {
          if (car.id === info.car_id) {
            return (
              <div key={info.id} className="flex md:flex-row md:w-full w-[310px] flex-col items-center justify-between m-auto shadow-md rounded-lg mb-4 md:m-5 md:h-[80px] px-[10px]">
                <div className="md:w-[100px] md:h-[80px] md:rounded-full p-2">
                  <img src={car.image} alt={car.name} className="w-full h-full object-cover rounded-lg md:rounded-full" />
                </div>
                <div className="flex flex-row flex-wrap gap-12 ml-8 mt-5 mb-4 md:flex-row md:justify-between w-full md:mx-11">
                  ...
                  <div className="flex flex-col justify-center">
                    <h1 className="text-md font-bold">{(car.daily_rate * (new Date(info.due_date).getTime() - new Date(info.reservation_date).getTime())) / (1000 * 3600 * 24)}$</h1>
                    <p className="text-sm text-gray-500">Total</p>
                  </div>
                </div>
                <div className="flex flex-col justify-center mb-[1rem]">
                  <button className="bg-gray-100 text-[#313131] px-4 py-2 rounded-lg" onClick={() => handleDelete(info.id)}>
                    Cancel
                  </button>
                </div>
              </div>
